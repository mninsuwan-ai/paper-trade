...
Workflow รันเอง **รอบเดียว** วันจันทร์–ศุกร์ (UTC) เวลา **22:07 UTC ≈ 05:07 น. ไทยของเช้าวันถัดไป**
ตลาดสหรัฐปิด 20:00 UTC (ฤดูร้อน) / 21:00 UTC (ฤดูหนาว) ข้อมูลจึงออกแล้วแน่นอน
ไม่ต้องเปิดคอมพิวเตอร์ทิ้งไว้

> **ทำไมไม่ตั้งตอน 08:00 น. ไทย** — GitHub รัน cron สายเป็นปกติ ถ้าตั้งไว้เวลาเดียวกับที่เราเปิดดู
> มันจะดูเหมือนพังทุกเช้าทั้งที่แค่ยังไม่ถึงคิว ตั้งไว้ 05:07 เผื่อ buffer ~3 ชั่วโมง
...
**เช็คว่าอัพเดทหรือยัง** ดูบรรทัด `Prices as of` บนหัวหน้าเว็บ ไม่ใช่ดูว่าตัวเลขเปลี่ยนไหม
(วันที่ตลาดนิ่ง ตัวเลขแทบไม่ขยับ) และกด Ctrl+F5 เสมอ เพราะเบราว์เซอร์ cache หน้า HTML ไว้
...
| `history_from` | (ทางเลือก) ให้สร้างกราฟย้อนหลังตั้งแต่วันที่นี้ — ดูหัวข้อถัดไป |

### กราฟย้อนหลัง (`history_from`)

พอร์ตที่ลอกมาจากบัญชีจริงมีจำนวนหุ้นคงที่อยู่แล้ว มูลค่าของวันไหนในอดีตจึงเท่ากับ
`จำนวนหุ้น × ราคาปิดวันนั้น` ตรงๆ ใส่ `"history_from": "2026-07-01"` แล้วระบบจะสร้าง
history รายวันย้อนหลังให้จากราคาที่ดึงมาอยู่แล้ว ไม่ต้องรอสะสมทีละวัน

- `track_since` กับ `track_base` จะถูกตั้งเป็นวันแรกที่มีข้อมูลครบทุกตัว
- จำนวนหุ้น SPY คำนวณใหม่ให้เริ่มที่เงินก้อนเดียวกันวันเดียวกัน — สองเส้นจึงออกจากจุดเดียวกัน
- เขียนใหม่ทั้งชุดทุกครั้งที่รัน (idempotent) รันซ้ำกี่รอบจำนวนจุดก็เท่าเดิม
- ย้อนได้ไกลสุดเท่าที่แหล่งข้อมูลให้มา — Alpha Vantage `outputsize=compact` = ~100 วันทำการ
